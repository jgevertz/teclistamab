%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                       %
% Sensitivity analysis code for teclistimab trimer model.               %
% Updated: 3/21/2025.                                                   %
...
%   - frequency = 2 weeks: dose = 1.5, 3.5, 6.0                         %
% - Run time (home computer):  5147.219215 (freq = 1, dose = 1.8)        %
%                             to 16283 sec  (freq = 2, dose = 3.5)      %
% - Update on 3/21/2025 removed some PK parameters from the analysis    %
%   and makes all corrections specified in trimer_teclistimab_new.m     %
%                                                                       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

clearvars; clc; close all; tic;
options=odeset('RelTol',1.0e-6); 
fraction_change = 0.20; % Looking for AUC(deathterm) to go DOWN by 
                        % 100*fraction_change%
increment = 0.01; % How much we perturb parameter by at each iteration of 
                  % algorithm (use 0.1 for a quicker run)
...
dfile = ['sensitivity_output_freq' num2str(frequency) 'wk_dose' ...
    num2str(dose) '.out'];
...
% pre-equilibrate
ICs = equilibrate_system(ICs, p, options);
% Now solve post-equilibration
...
exclude_params = {'doseD','iv', 'firstDoseTime', 'LD1', 'LD2', ...
    ...
    'cl_tvar','kdes','KD_cd3','KD_BCMA','equilibrate','koff3bm',...
    'koff3p','kon3p','koff_sB','koff_mB','p.ksyn_th'}; 
...
IC_names = {'CD3th0', 'CD3p0', 'sBp0', 'Dsc0', 'Dper0', 'Dp0', 'D3p0',...
    ...
    'DsBbm0', 'DmBbm0', 'DsB3bm0', 'DmB3bm0','koff_mB','koff_sB'};
include_ICs = [1 2 3 10 11 12]; % these index the nonzero ICs: we only 
                                % analyze these
...
for i = 1:length(labels2)
    ...
    if check_label == 0  
        labels2(i) = ''; 
    elseif check_label>0
        % Label should have plus/minus
        if flag_equal_pert(length(param_names)+i) == 1 
...
save(fname, 'fraction_change','increment','dose','frequency',...
    'AUC_deathterm_baseline', 'param_names','IC_names','AUC_deathterm',...
    'min_paramChange','param_IC_change','flag_equal_pert','ymin_plot',...
    'ymax_plot','num_params_ICs','include_ICs');
toc;
diary off
...
function [tDose, cDose, ti] = set_protocol(p, tmax, doses,frequencies)
    ...
    if (p.LD1 > 0) && (p.LD2 > 0) 
        ...
        for i = 1:p.dosenumD
            ...
            tDose(i + 2) = p.firstDoseTime + (2*p.intvlLD) + (intvlD*(i-1));
            cDose(i + 2) = doseD;
        end
